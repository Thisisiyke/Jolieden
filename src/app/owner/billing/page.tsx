...
export default function OwnerBillingPage() {
  const [tab, setTab] = useState<"history" | "accounts">("history");
  return (
    <>
      <PageHeader title="Billing" />
      <SubTabs<"history" | "accounts">
        tabs={[
        ...
        {tab === "history" ? (
              ...
              <tbody>
                {INVOICES.map((iv) => (
                  <tr key={iv.id} className="border-t border-ink-100 hover:bg-ink-50">
                    <td className="px-4 py-3 font-medium">{iv.id}</td>
          ...
          <Card>
            <div className="p-5 flex items-center justify-between">
              ...
              <button className="h-9 px-3 rounded border border-ink-300 text-[14px] font-medium text-ink-700 hover:bg-ink-50">Update card</button>
            </div>
